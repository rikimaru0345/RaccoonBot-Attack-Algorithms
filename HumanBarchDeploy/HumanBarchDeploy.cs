...
namespace HumanBarchDeploy
{
    ...
    internal class HumanBarchDeploy : BaseAttack
    {
        ...
        private int _minimumExposedTargets = 2; //Was 6
        ...
        public override double ShouldAccept()
        {
            ...
            if (Opponent.IsDead(true))
            {
                ...
                if (IgnoreGold)
                {
                    Log.Info($"{Tag}Minimum Gold = 0  - Ignoring Gold Storages/Collectors");
                ...
                if (IgnoreElixir)
                {
                    Log.Info($"{Tag}Minimum Elixir = 0  - Ignoring Elixir Storages/Collectors");
                    minTargets = (int)Math.Floor(Convert.ToDouble(minTargets) / 2d); ;
                }
                if (minTargets == 0)
                ...
                Log.Info($"{Tag}{ripeCollectors} targets found outside walls. Min={minTargets}");
            ...
            else
            {
                ...
                if (townHall.CanSnipe())
                {
                    ...
                    Log.Info($"{Tag}Sniping Active Town Hall!");
                    return 1;
                }
                else
                {
                    Log.Warning($"{Tag}Skipping Active Base, TH is not Snipable.");
        ...
        public override IEnumerable<int> AttackRoutine()
        {
            ...
            while (true)
            {
                ...
                if (!isDead)
                {
                    if (townHallTarget.ValidTarget)
                    {
                        ...
                        if (gruntUnits.Any())
                        {
                            ...
                            Log.Info($"{Tag}Sniping Town Hall {gruntsToDeploy} Grunts Near: X:{townHallTarget.DeployGrunts.X} Y:{townHallTarget.DeployGrunts.Y}");
                        ...
                        if (rangedUnits.Any())
                        {
                            ...
                            Log.Info($"{Tag}Sniping Town Hall {rangedToDeploy} Ranged Near: X:{townHallTarget.DeployRanged.X} Y:{townHallTarget.DeployRanged.Y}");
                        ...
                        { 
                            ...
                            {
                                Log.Info($"{Tag}Deploying King at: X:{townHallTarget.DeployGrunts.X} Y:{townHallTarget.DeployGrunts.Y}");
                            ...
                            {
                                Log.Info($"{Tag}Deploying Queen at: X:{townHallTarget.DeployRanged.X} Y:{townHallTarget.DeployRanged.Y}");
                ...
                else
                {
                    ...
                    Target[] targets = HumanLikeAlgorithms.GenerateTargets(_minimumAttackDistanceToCollectors, IgnoreGold, IgnoreElixir, collectorCacheBehavior, outputDebugImage);
                    ...
                    if (collectorCount < 1)
                    {
                        Log.Info($"{Tag}Surrendering - Collectors Remaining = {collectorCount}");
                    ...
                    for (int p = 0; p < groupedTargets.Count; p++)
                    {
                        ...
                        for (int i = 0; i < groupedTargets[p].Length; i++)
                        {
                            ...
                            if (gruntUnits.Any())
                            {
                                ...
                                Log.Info($"{Tag}{gruntsAtCollector} Grunts Around Point: X:{gruntDeployPoint.X} Y:{gruntDeployPoint.Y}");
                        ...
                        if (secondLargestSetIndex == p && secondLargestSetCount >= 3) {
                            ...
                            if (UserSettings.UseKing && king.Any())
                            {
                                Log.Info($"{Tag}Deploying King at: X:{groupedTargets[p][1].DeployGrunts.X} Y:{groupedTargets[p][1].DeployGrunts.Y}");
                        ...
                        if (largestSetIndex == p && largestSetCount >= 3)
                        {
                            ...
                            if (UserSettings.UseQueen && queen.Any())
                            {
                                ...
                                Log.Info($"{Tag}Deploying Queen at: X:{groupedTargets[p][1].DeployRanged.X} Y:{groupedTargets[p][1].DeployRanged.Y}");
                        ...
                        for (int i = 0; i < groupedTargets[p].Length; i++)
                        {
                            ...
                            if (rangedUnits.Any())
                            {
                                ...
                                Log.Info($"{Tag}{rangedAtCollector} Ranged Around Point: X:{rangedDeployPoint.X} Y:{rangedDeployPoint.Y}");
                ...
                if (isDead)
                {
                    if (postLoot.Gold + postLoot.Elixir + postLoot.DarkElixir >= 0)
                    {
                        ...
                        {
                            Log.Info($"{Tag}Surrendering because gained resources isn't enough");
                            break;
                        }
                        preLoot = postLoot;
                    }
                ...
                else
                {
                    if (Attack.HaveAStar())
                    {
                        Log.Info($"{Tag}We have a star! TH Sniped!");
                        ...
                        if (newGold + newElixir < (preLoot.Gold + preLoot.Elixir) * .05f) //Less than 5% of what is available.
                        {
                            ...
                            Log.Info($"{Tag}Not much loot gained from Snipe(G:{newGold} E:{newElixir} out of G:{preLoot.Gold} E:{preLoot.Elixir}) - Try to Loot Collectors also...");
                            isDead = true;
                        }
                        else
                        {
                    ...
                    if (waveCounter > 10)
                    {
                        Log.Info($"{Tag}Fail! TH Not Sniped! our troops died - Surrendering...");
                        break;
                    }
                }
